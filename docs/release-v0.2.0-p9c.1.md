...
`.1` 推送后的清理日志检查发现：浏览器验收资产曾被真实项目引用后移除，清理不能物理删除该资产。已恢复并验证 TIFF/COG/缩略图，外部历史引用保留；无外部 Job/Prompt 使用该测试资产。清理脚本改为先锁定资产、检查全部外部引用，再保留有关联资产及其文件。该修正归档为 **v0.2.0-p9c.2**，不覆盖 `.1`。
