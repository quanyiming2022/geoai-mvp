...
export async function languageRequest(path:string,body?:unknown,method='POST'){
 await requireUser();
 if(!(/^\/admin\/llm(?:\/test)?$/.test(path)||/^\/llm\/availability$/.test(path)||/^\/projects\/[0-9a-f-]{36}\/assistant\/(plan|confirm|window)$/.test(path)))throw new Error('Invalid assistant route');
