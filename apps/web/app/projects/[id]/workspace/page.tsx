...
import { type AvailableEndpoint } from '../../../../components/tile-extraction-form';
...
export default async function Workspace({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ message?: string; job?: string; jobpage?: string }> }) {
 ...
 return <>{message && <p role="status">{message}</p>}<RasterPoll monitorEndpoints pending={assets.some(a=>a.status === 'uploaded' || a.status === 'processing') || jobs.some(j=>j.status==='queued'||j.status==='running')}/><ProjectMap endpoints={endpoints} projectName={project.name} userEmail={user.email} hasHealthyEndpoint={endpoints.some(e=>e.health_status==='healthy')} jobs={jobs} key={id} rasters={assets} aois={aois} prompts={prompts} results={results} projectId={id} editable={project.my_role!=='viewer'} dataPanel={<>{project.my_role!=='viewer'&&<RasterUpload projectId={id}/>}{assets.filter(a=>a.status==='failed').map(asset=><form key={asset.id} action={retryRaster}><span>{asset.filename}: {asset.error_code}</span><input type="hidden" name="project_id" value={id}/><input type="hidden" name="asset_id" value={asset.id}/>{project.my_role!=='viewer'&&<button className="secondary">重试处理</button>}</form>)}</>} mockExtractionPanel={<ExtractionForm prompts={prompts} aois={aois} rasters={assets} projectId={id} idempotencyKey={randomUUID()}/>} extractionKey={randomUUID()} resultsReviewPanel={<ResultsPanel results={results} projectId={id} editable={project.my_role!=='viewer'}/>} jobsPanel={<JobPanel page={page} jobs={jobs} projectId={id} editable={project.my_role!=='viewer'}/>}/></>;
}
