...
export default function TaskAssistant({projectId,editable,onConfirmed,context,onContextChange,rasters,prompts,aois,endpoints}:{projectId:string;editable:boolean;onConfirmed?:()=>void;context:AssistantContext;onContextChange:(c:AssistantContext)=>void;rasters:{id:string;filename:string;status:string}[];prompts:Named[];aois:Named[];endpoints:AvailableEndpoint[]}){
 ...
 function update(value:Partial<AssistantContext>){onContextChange({...context,...value});setAnswer(null);setConsentKey(null);setError('');}
 ...
 {context.channel==='mock'?choice('aoi_id','AOI 搜索范围',aois):<>{choice('endpoint_id','模型与计算节点',endpoints.filter(e=>e.health_status==='healthy').map(e=>({id:e.id,name:`${e.model_name} · ${e.name} · ${e.usage_policy==='research_only'?'仅研究用途':e.usage_policy}`})))}<div className="assistant-window">{(['query_col','query_row'] as const).map((field,i)=><label key={field}>{i?'起始行':'起始列'}<input type="number" min={0} step={1} disabled={busy} value={context[field]??''} placeholder="源像素" onChange={e=>update({[field]:e.target.value===''?undefined:Number(e.target.value)})}/></label>)}</div><small>固定 512×512 源像素。此窗口不是 AOI 全范围；不支持大图扫描。</small></>}
