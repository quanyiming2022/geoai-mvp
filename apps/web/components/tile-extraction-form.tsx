...
export default function TileExtractionForm({projectId,prompts,rasters,endpoints,idempotencyKey}:{projectId:string;prompts:VisualPrompt[];rasters:RasterAsset[];endpoints:AvailableEndpoint[];idempotencyKey:string}) {
 ...
 useEffect(()=>{const receive=(event:Event)=>{const d=(event as CustomEvent).detail;if(d.raster_id===rasterId){setCol(String(d.query_col));setRow(String(d.query_row));setSelecting(false);}};window.addEventListener('workspace-tile-selected',receive);return()=>window.removeEventListener('workspace-tile-selected',receive);},[rasterId]);
 ...
 {step===0&&<><label>目标影像<select value={rasterId} onChange={e=>{setRasterId(e.target.value);setCol('');setRow('');}}>{usable.map(r=><option key={r.id} value={r.id}>{r.filename} · {r.width} × {r.height}</option>)}</select></label>{!usable.length&&<p role="status">请先上传至少 512 × 512 的 RGB 影像，等待处理完成。</p>}</>}
 ...
 {step===2&&<><button type="button" className="secondary" disabled={!raster} onClick={()=>{setSelecting(true);window.dispatchEvent(new CustomEvent('workspace-select-tile',{detail:{rasterId,origin:'manual'}}));}}>在地图上选择单瓦片测试区域</button><p>{selecting?'在地图影像上点击测试位置，Esc 取消。':windowValid?'已选择测试区域 · 512×512 原分辨率像素':'尚未选择测试区域'}</p><small>系统自动计算源像素窗口；不执行 AOI 扫描。</small></>}
